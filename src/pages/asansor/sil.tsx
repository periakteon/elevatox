...
import { columns } from "@/components/tanstack-table/firma-asansor/sil/columns";
import { DataTable } from "@/components/tanstack-table/firma-asansor/sil/data-table";
import {
  Card,
  CardContent,
  CardDescription,
...
const DeleteElevator: MyPage = () => {
  ...
  const { isLoading, isError, isLoadingError, error, data } =
    api.elevator.getElevatorListToDelete.useQuery();
  ...
  return (
    <>
      <Head>
        <title>Asansör Sil | {companyInfo?.companyInfo?.name} - ElevatoX</title>
      </Head>
      <div className="p-4 sm:ml-64">
        <Card className="mt-16 rounded-lg p-0">
          <CardHeader>
            <CardTitle>Asansör Sil</CardTitle>
            <CardDescription className="w-full">
              Aşağıdaki listeden asansör silme işlemini gerçekleştirebilirsiniz.
            </CardDescription>
          </CardHeader>
          <CardContent>
...
export default DeleteElevator;
DeleteElevator.Layout = "Main";
