...
export interface KuyuDibiMaintenanceByParams {
  staff: {
    firstName: string;
    lastName: string;
  };
...
interface DataTableProps<TData extends KuyuDibiMaintenanceByParams, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
}

export function DataTable<TData extends KuyuDibiMaintenanceByParams, TValue>({
  columns,
  data,
  ...
  const deleteMultipleMaintenance =
    api.maintenance.deleteMultipleKuyuDibiMaintenance.useMutation({
      onMutate: () => {
        emitter.publish(
          "showNotification",
          "Kuyu dibi bakım kaydı siliniyor...",
        );
      },
  ...
  const exportToCSV = (maintenanceData: string[][], fileName: string) => {
    ...
    const wb: XLSX.WorkBook = {
      ...
      Props: {
        ...
        Language: "tr-TR",
        Keywords: `Kuyu Dibi Bakım Kaydı Listesi, ${new Date().toLocaleDateString()}`,
      ...
      <div className="mt-4 block">
        <Button
          ...
          onClick={() => {
            ...
            if (selectedData.length) {
              exportToCSV(
                selectedData,
                `${new Date().toLocaleDateString()} - Seçili Kuyu Dibi Bakım Kaydı Listesi`,
              );
            }
          }}
        >
        ...
        <Button
          ...
          onClick={() => {
            ...
            exportToCSV(
              allData,
              `${new Date().toLocaleDateString()} - Kuyu Dibi Bakım Kaydı Listesi`,
            );
          }}
        >
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                ...
                adet kuyu dibi bakım kaydını silmek istediğinize emin misiniz?
              </AlertDialogTitle>
                ...
                onClick={() => {
                  ...
                  if (selectedData.length) {
                    deleteMultipleMaintenance.mutate({
                      ...
                      kuyuDibiMaintenanceIds: selectedData.map(
                        (data) => data.id,
                      ),
                    });
                  }
