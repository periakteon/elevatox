...
import { kuyuDibiMaintenanceSchema } from "@/utils/schemas";
...
export default function KuyuDibiMaintenanceForm({
  buildingName,
  maintenanceId,
  ...
  const createMaintenance =
    api.maintenance.createKuyuDibiMaintenance.useMutation({
      onMutate: () => {
        emitter.publish(
          "showNotification",
          "Kuyu dibi bakım kaydı oluşturuluyor...",
        );
      },
  ...
  function onSubmit(data: z.infer<typeof kuyuDibiMaintenanceSchema>) {
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel>Hız Regülatörü</FormLabel>
                    </div>
                  </FormItem>
                )}
              />
              ...
              todayMaintenanceList.data?.makineDairesiMaintenance?.length >
                0 ? (
                <>
                  <Button
                    variant={"link2"}
